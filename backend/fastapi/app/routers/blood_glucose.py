...
SPRING_INTERNAL_URL = os.getenv("SPRING_INTERNAL_URL", "http://springboot:8080")
...
async def _trigger_glucose_alert(user_id: str, record: "BloodGlucoseCreateRequest") -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            await client.post(
                f"{SPRING_INTERNAL_URL}/api/internal/glucose-alerts",
                json=payload,
                headers={"X-Internal-Api-Key": INTERNAL_API_KEY},
            )
    except httpx.RequestError as exc:
        logger.warning("혈당 알림 발송 실패 (user=%s level=%s): %s", user_id, record.level, exc)
...
async def save_blood_glucose(user_id: str, date: str, record: BloodGlucoseCreateRequest):
    ...
    await _trigger_glucose_alert(user_id, record)
