...
## [2026-06-18] 삼성 헬스/Health Connect 연동을 프리미엄(PAID) 구독자로 제한

### 배경
기존에는 구독 상태(`SessionHolder.tier`)와 무관하게 누구나 라이프스타일 화면에서 삼성 헬스/Health Connect 연동이 가능했음. 프리미엄 구독 선택 시에만 헬스 연동을 제공하도록 게이팅 요청.

### 작업 내용
| 파일 | 변경 |
|------|------|
| `ui/lifestyle/LifestyleFragment.kt` | `trySamsungSilent()` — `SessionHolder.tier != UserTier.PAID` 면 silent 연동 시도 자체를 스킵. `onRefreshClicked()` — PAID 가 아니면 Samsung/Health Connect 시도 전에 토스트("프리미엄 구독자만 이용 가능") 노출 후 `SubscriptionActivity` 로 이동, 연동 로직 미실행 |

### 범위 / 미포함
- **신규 연동 차단만 구현.** 이미 PAID 상태에서 연동했다가 구독이 해지(FREE/GUEST로 다운그레이드)되는 시나리오의 기존 연동 강제 해제는 범위 밖(사용자 확인 후 제외 결정). 앱은 메모리 기반 상태라 재시작 시 Mock으로 초기화됨.
- 빌드 검증: `compileDebugKotlin` BUILD SUCCESSFUL.

---

## [2026-06-18] 라이프스타일 화면 — 비구독자에게 헬스 연동 UI 전체 숨김 (추가 후속 작업)

### 배경
직전 작업(연동 로직 게이팅)만으로는 비구독 상태에서도 동기화 배너(고정 텍스트 "삼성 헬스에서 자동 동기화됨")·운동/식사/수면 카드 UI가 그대로 노출되어 사용자에게 오해를 줄 수 있음(실기 스크린샷으로 확인). 비구독 시에는 UI 자체를 안내 텍스트로 대체하도록 요청받음.

### 작업 내용
| 파일 | 변경 |
|------|------|
| `res/layout/fragment_lifestyle.xml` | 동기화 배너 + 운동/식사/수면 3개 카드를 `LinearLayout(id=group_health_content)`로 묶음. 그 앞에 `TextView(id=tv_subscribe_required, 기본 visibility=gone)` "구독 시 삼성헬스와 연동해서 이용 가능합니다" 추가 |
| `ui/lifestyle/LifestyleFragment.kt` | `applyTierGate()` 신설 — `SessionHolder.tier == PAID` 면 `group_health_content` 표시/`tv_subscribe_required` 숨김, 아니면 반대. `onViewCreated`에서 1회, **`onResume`에서 매번** 호출(구독 화면에서 결제 후 복귀 시에도 즉시 갱신되도록). `trySamsungSilent()` 호출도 `onViewCreated`→`onResume`으로 이동(탭 재방문 시에도 재시도) |

### 주요 결정
- PAID 전환 시점에 헬스 연동 UI가 즉시 나타나도록 fragment view 재생성 없이도 `onResume` 기준으로 매번 게이트 재평가.
- 빌드 검증: `compileDebugKotlin` BUILD SUCCESSFUL.
